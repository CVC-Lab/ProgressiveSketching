...
# Input/output paths
video_path = "/content/drive/MyDrive/dropbox_copy/Hongik/Research/P-SCT-StructuralEng/data/Tacoma Bridge.mp4"

# Load video with moviepy to clip between 1:11 and 2:00 (in seconds)
start_time = 72
end_time = 82
clip = VideoFileClip(video_path).subclip(start_time, end_time)

# Extract frames as grayscale numpy array
fps = clip.fps
frames = [cv2.cvtColor(f, cv2.COLOR_RGB2GRAY) for f in clip.iter_frames(fps=fps, dtype="uint8")]
h, w = frames[0].shape

A = np.array(frames)  # Shape: (num_frames, H, W)
...
for it in tqdm(range(100)):
    ...
    np.save(f'/content/drive/MyDrive/dropbox_copy/Hongik/Research/P-SCT-StructuralEng/outputs/Tacoma_err_log_PSCT_{it}.npy',np.array(err_logPS))
    np.save(f'/content/drive/MyDrive/dropbox_copy/Hongik/Research/P-SCT-StructuralEng/outputs/Tacoma_err_log_RSCT_{it}.npy',np.array(err_logRS))
    np.save(f'/content/drive/MyDrive/dropbox_copy/Hongik/Research/P-SCT-StructuralEng/outputs/Tacoma_memory_log_PSCT_{it}.npy',np.array(memory_logPS))
    np.save(f'/content/drive/MyDrive/dropbox_copy/Hongik/Research/P-SCT-StructuralEng/outputs/Tacoma_memory_log_RSCT_{it}.npy',np.array(memory_logRS))
    np.save(f'/content/drive/MyDrive/dropbox_copy/Hongik/Research/P-SCT-StructuralEng/outputs/Tacoma_nf_PSCT_{it}.npy',np.array(nf_PS))
    np.save(f'/content/drive/MyDrive/dropbox_copy/Hongik/Research/P-SCT-StructuralEng/outputs/Tacoma_nf_RSCT_{it}.npy',np.array(nf_RS))
